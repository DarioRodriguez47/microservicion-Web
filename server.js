...
app.use((req, res, next) => {
  ...
  console.log(`[${timestamp}] ${req.method} ${req.path} - Content-Type: ${req.headers['content-type']}`);
  next();
});
...
app.post('/api/songs', checkConnection, async (req, res) => {
  try {
    ...
    const createdSong = await songsCollection.findOne({ _id: result.insertedId });
...
app.put('/api/songs/:id', checkConnection, async (req, res) => {
  try {
    ...
    const existingSong = await songsCollection.findOne({ _id: new ObjectId(id) });
    if (!existingSong) {
      return res.status(404).json({ 
        success: false,
        error: 'Canción no encontrada' 
    ...
    const updatedSong = await songsCollection.findOne({ _id: new ObjectId(id) });
